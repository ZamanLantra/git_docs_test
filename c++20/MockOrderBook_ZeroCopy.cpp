...
public:
    ...
    void update(uint64_t order_id, int new_quantity) {
        ...
        if (ord->side == Side::BUY) {
            update_price_level<Side::BUY>(ord->price, (new_quantity - ord->quantity));
        } else {
            update_price_level<Side::SELL>(ord->price, (new_quantity - ord->quantity));
        }
    ...
    void cancel(uint64_t order_id) {
        ...
        Order* ord = order_map[order_id];
        if (ord->side == Side::BUY) {
            update_price_level<Side::BUY>(ord->price, -ord->quantity);
        } else {
            update_price_level<Side::SELL>(ord->price, -ord->quantity);
        }
...
private:
    ...
    void update_price_level(double price, int update_quantity) {
        ...
        if constexpr (side == Side::BUY) {
            ...
            if (idx == best_ask_index && ask_levels[idx] == 0) {
                for (size_t i = idx + 1; i < MAX_PRICE_LEVELS; ++i) {
                    if (ask_levels[i] > 0) {
                        best_ask_index = i;
                        return;
                    }
    ...
    size_t best_bid_index;
    size_t best_ask_index;
};
...
🟢 Insert Time: 40 ms → 2.5e+07 ops/sec
🟡 Update Time: 18 ms → 5.55556e+07 ops/sec
🔴 Cancel Time: 24 ms → 4.16667e+07 ops/sec
...
int main() {
    ...
    OrderBook book;
    try {
        auto o1 = Order{1, 100.0, 10, Side::BUY};
        auto o2 = Order{2, 101.0, 5, Side::SELL};
        book.insert(&o1);
        book.insert(&o2);
        book.update(1, 15);
        book.cancel(2);
